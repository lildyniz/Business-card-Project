...
def index(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            file.save()
            return redirect('tf-idf:file_to_analysis', id=file.id)
    else:
        form = FileForm()
...
def delete_all_data(request):
    ...
    # Удаление всех файлов из папки media/tf-idf/
    tf_idf_path = os.path.join(settings.MEDIA_ROOT, 'tf-idf')
    if os.path.exists(tf_idf_path):
        shutil.rmtree(tf_idf_path)
    return HttpResponseRedirect(reverse("tf-idf:index")) 